...
class PendingSync(ctx: Context, params: WorkerParameters) : CoroutineWorker(ctx, params) {
    ...
    override suspend fun doWork(): Result {
        ...
        if (Store.token(ctx) == null) {
            // Unpaired while things were queued. Nothing here can ever be delivered, and
            // holding it would resurrect old spends against whatever ledger is paired next.
            Store.writePending(ctx, emptyList())
        ...
        for (item in Store.pending(ctx)) {
            ...
            when (r) {
                ...
                Api.Result.Unpaired -> { Store.forget(ctx); Store.writePending(ctx, emptyList()); return Result.success() }
                Api.Result.Offline -> { left += item; offline = true }
            }
        }
        Store.writePending(ctx, left)
        LedgerTileService.refresh(ctx)
