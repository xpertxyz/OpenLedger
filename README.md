...
- **Expense** tab — the month's spending grouped by day with per-day totals + monthly category breakdown. Swipe left/right to change month.
- **Per-category monthly budgets** — set a limit per category and Expense shows spent / % used / left, turning red when you go over. Budgets **never block a spend**; they report, they don't police.
...
- **Earnings** grouped by month with a per-category breakdown (Salary, Interest, Other by default — add, rename and delete your own). The Earn tab summarises **this month / year to date / all time** and carries a rolling twelve-month **earned vs spent vs invested** chart, with a pie of the same window underneath giving each of the three its amount and share of the total.
- **Investments** grouped by month with per-type breakdown (SIP, Stocks, FD-RD, Gold, PPF-EPF, and any custom types you add).
- **Two views of Invest, one toggle** — *All time* is the default: everything ever put in, Active / Archived / Total, and a twelve-month strip. *Monthly* is the Expense tab's shape applied to the same data — one month at a time with a day strip, per-type bars against their targets, a type filter and the month's entries grouped by day. Swipe left and right to change month. The person filter and the Active/Archived filter scope both, and the toggle keeps whatever you had set.
- **Per-type monthly targets** — the investment side of a budget, and the opposite kind of number: a budget is a ceiling you would rather stay under, a target is a floor you are trying to reach. Set one on a type and Invest shows *this month* against it, what's left to go, and *target met* once you're there. Falling short is never coloured red — missing a target isn't the same news as going over a budget.
- **Sub-types, one level deep** — nest "Nifty 50" and "Small cap" under "SIP". Their money rolls up into the parent's bar and counts towards the parent's target, with each child listed beneath it (plus a *Direct* line for the parent's own money, so the lines always sum to the bar). Sub-types carry no target of their own.
- **Organise investment types** screen — the mirror of the expense one, and the only place types are managed: rename, target, nest, archive, delete, add, and merging one type's entries into another (recurring items follow, so nothing keeps posting into the type you just emptied). Investments naming a type the ledger no longer has are counted there as **Unrecognised** and can be filed into a real one.
...
- **Filter by person** — All / Me / Arjun pills at the top of Expense, Earn, Invest and the yearly summary, above everything they scope. Every total, bar and breakdown on the page follows the pick, and it survives month and year navigation. Only appears once a ledger is actually shared; on Expense, Earn and Invest the **+ Add** button shares that row, so a solo ledger still gets the row for the button alone.
- **"Me" means whoever is reading** — a name is shown from the viewer's side of the ledger, not the owner's. Your own row always reads *Me* whatever it is stored as, somebody else who signs in reads as their own account name, and a row nobody signs in as ("Appa", "Kids", a shared card) keeps whatever the household called it. Your row sorts first, so *Me* sits in the same place for everyone.
- **The owner files on anyone's behalf** — the person who keeps the books logs half of what the household spends, so they can name any member on an expense, earning, investment or recurring item. Everybody else files as themselves, whatever their form posts.
- **Editable everything** — every list row opens an inline edit modal, and on Expense the same dialog adds a new one without leaving the month you're reading. Rename a category or an investment type on its Organise screen and it reflects app-wide, entries and recurring items included.
...
- **Themed dropdowns, list and all** — a `<select>` opens the app's own list rather than the OS widget, so the same tap looks the same in the browser and inside the Android WebView. The `<select>` is still what holds the value and submits, so nothing depends on the JavaScript being there.
...
households ─┬─ users        (Google-authenticated, one per person)
            ...
            │                       `target` = monthly aim, 0 = none; `parent_id` = sub-type,
            │                       one level, always target 0)
...
- **A split bill is a recurring item with a stop date, not a new entity.** "Split a bill" on the Recurring tab divides one prepaid total into equal monthly shares and stores a single ordinary `kind = 'expense'`, `frequency = 'monthly'` row whose `next_date` is the day you paid and whose `end_date` is the last instalment. The normal sweep does the rest: it back-fills every month from the payment date to today in one pass and then stops, because both the sweep's probe and its loop test `end_date`. Two consequences worth stating. First, the shares are rounded to the paisa, so an uneven division (10,000 over 12 months) can miss the total by a few paise — the dialog previews `per × months` before you save rather than reporting the drift afterwards. Second, month arithmetic clamps: `addMonths()` turns Jan 31 + 1 month into Feb 28, where PHP's own `+1 month` returns Mar 3 and silently skips February. `advanceDate()` routes through it, so *every* monthly recurring item posts twelve times a year, split or not.
...
- **Signed out, there are exactly four public routes.** `/` renders the landing page, `/login` the Google gate, `/terms` the terms, and `POST /signin` takes Google's callback. Every other path redirects to `/login` rather than quietly rendering the gate in place, so a signed-out deep link doesn't sit at a URL that will mean something else once you sign in. Signed in, `/login` redirects to `/`; signing out lands on `/login`. Both public pages default to the OS dark preference via `prefers-color-scheme` — a visitor has no `users` row to read. The landing page's theme button overrides that, stored in `localStorage` as `ol-theme` + `ol-palette` and replayed by an inline `<head>` script on both pages so an explicit choice never flashes the other theme. The media query is scoped to `:root:not([data-theme])`, which is what lets an explicit *light* choice win on a dark OS; without it the query would outrank the stored preference. No stored value means system, and that path needs no JavaScript.
- **A theme is two attributes on `<html>`, and every palette ships in the page.** `THEMES` in `views.php` holds three palettes x light/dark; `themeCss()` writes all six as `[data-palette][data-theme]` blocks, so switching is `data-palette` + `data-theme` rewritten in place — instant, nothing fetched, nothing re-rendered. The `POST /theme` that follows only records the choice for the next page and answers `204`; losing it costs a preference, not the page you were on. Which card reads as selected is CSS off the same two attributes, so the picker cannot disagree with the app around it. Harbor and Plum are generated in OKLCH on the **same lightness scale and chroma arc as Organic** — only the hues move, which is why every component works untouched across all three: `--color-accent-700` carries the same visual weight whichever palette is on. Organic needs no block of its own for light: `design-tokens/styles.css` `:root` *is* Organic, so "no attribute" already renders it, and preflight checks that every other palette defines the full variable set (a missing one wouldn't fall back — the previous palette's value would stay on the page).
- **"Uncategorised" has one definition.** `category_id IS NULL` *or* pointing at a row that no longer exists — both render identically, so `uncategorisedWhere()` in `lib.php` is the single predicate the Expense bucket, the on-screen count, the file-away move and the bulk delete all share. Two definitions would mean the number on the button disagrees with what it touches.
...
- **Investment types are referenced by name, not by id** (`investments.type` and `recurring.type` both store the string). That one difference from `categories` shapes every tool on the Organise investment types screen: a rename has to cascade into *both* fact tables, moving entries rewrites the name rather than repointing an FK, and two types may not share a name — `typeNameTaken()` refuses it, because a duplicate would make the rollup, the archive filter and the rename cascade all ambiguous. A row can still name a type this ledger no longer has (a restore from an older backup will do it); `unknownTypeWhere()` is the single predicate behind the **Unrecognised** count and the move that files it away, the same way `uncategorisedWhere()` works for expenses.
- **Sub-types mirror sub-categories, and roll up by name.** `investment_types.parent_id` is the same one-level rule with the same target-clearing on nest. `rollupTypes()` folds a child's money onto its parent for the Invest bars, but only when the parent passes the *same* archived/active filter — otherwise an "active" view could print an archived type's name over somebody else's money.
- **A target is measured against the month, not the lifetime.** In the all-time view the Invest bars total everything ever put in, which no monthly figure can be compared to, so a type carrying a target gets one extra grouped scan of the current month to sit against it. The monthly view needs no such scan — its bars and its targets already measure the same window.
- **The Invest views are two renderers, not one with branches.** `renderInvest()` answers "what have we built up" and `renderInvestMonth()` answers "what did we put in this month"; they share the data helpers and `investDialogs()`, because the two differ in what they summarise and never in how you enter or correct an entry. `?v=m` selects the monthly one and *nothing* selects the all-time one, so every link that predates the split still lands where it always did.
- **Archiving is per *type*, matched by name.** Archiving hides a type's entries from the active view and removes it from the *add* pickers, but keeps it in the *edit* pickers so existing entries stay editable.
...
- **Household scoping on writes**: category, type and member ids arrive from `<select>` fields, so every one is re-checked against the signed-in household before it is stored (`ownedId()`); a forged id degrades to *uncategorised* instead of attaching to another household's row. Attribution has a second rule on top of scoping — `attributableMember()` decides whose name an entry may carry, and a member's posted `member_id` is discarded in favour of their own linked row.
